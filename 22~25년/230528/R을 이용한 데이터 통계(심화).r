...
qqnorm(Energy$Energy.Consume[Ventilation.Machine == 1])
qqnorm(Energy$Energy.Consume[Ventilation.Machine == 1])

# 행을 분할하지 않고 정규성 검정 수행
ad.test(Energy$Energy.Consume[Ventilation.Machine == 1]) # 전기
ad.test(Energy$Energy.Consume[Ventilation.Machine == 2]) # 열
...
comp <- LSD.test(model, "group", p.adj = 'bonferroni')
...
# 상관분석(Spearman)
cor(scatter.plot$Work.Velocity, scatter.plot$Error.Rate, method="spearman")
cor.test(scatter.plot$Work.Velocity, scatter.plot$Error.Rate, method="spearman")
